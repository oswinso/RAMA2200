...
main:	lea $sp, initsp                         ! initialize the stack pointer
        ...
        addi $s2, $zero, 0x1DEAD                ! Address of SSEG

        jalr $ra, $at                           ! run pow
        lea $a0, ANS                            ! load base for pow
        sw $v0, 0($a0)

        sw $v0, 0($s2)                          ! Display recursive result on SSEG
...
POW:    addi $sp, $sp, -1                       ! allocate space for old frame pointer
        ...
        skplt $zero, $a1                        ! check if $a1 is zero (if not, skip the goto) 		      - Skip if a1 > 0
        goto RET1                               ! if the power is 0 return 1
        skplt $zero, $a0
        goto RET0                               ! if the base is 0 return 0 (otherwise, the goto was skipped) - Skip if a0 > 0
